...
export type translatedRstData = {
  translated_better_arry: translatedObjectType[]
}
...
export default async function translate(srtString: string, option: {}) {
...
const test = async () => {
  ...
  const pickedArr = jsonChunk.map((ele: any) => pick(ele, ['position', 'text']))
